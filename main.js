...
let gameEntered = false;   // flips in startGame: pre-entry failures are fatal, later ones are toasts
window.addEventListener('error', e => {
  loadMsg('ERROR: ' + e.message);
  // load/menu phase: a failed critical script/JSON/GLB is fatal -> styled #err.
  // in-game: never cover the running game, a non-blocking toast is enough.
  const text = String(e.message || '').replace(/^Uncaught (Error: )?/, '');
  if (!text) return;
  if (!gameEntered) showError('资源或脚本错误', text);
  else showMsg('发生错误（已记录）: ' + text, 2.2);
});
const failedAssets = [];
loadMgr.onError = url => {
...
const J = u => {
  ...
  return fetch(u)
    .then(r => {
      if (!r.ok) throw new Error(`加载失败: ${decodeURIComponent(u)} (HTTP ${r.status})`);
      return r.json();
    })
...
// Rejection triage: only PRE-ENTRY failures (critical scripts/JSON/GLB during
// load & menu) are fatal and earn the #err overlay. After entering the game a
// stray async error is logged + surfaced as a non-blocking toast — it must
// never cover the running game. preventDefault() keeps "Uncaught (in promise)"
// noise out of the console; errors already caught by safeRequestPointerLock
// never reach this handler at all.
window.addEventListener('unhandledrejection', e => {
  e.preventDefault();
  console.error('[unhandledrejection]', e.reason);
  if (!gameEntered) {
    showError('资源或脚本错误', String(e.reason?.message || e.reason || '未知错误'));
  } else {
...
import { buildCityBoxes } from './cityBoxes.js?v=7';
...
function onPointerLockFail(e) {
  console.warn('[pointer-lock] request failed:', e?.name || e?.message || e);
  if (phase === 'play' || phase === 'pause') showMsg('鼠标未锁定，点击游戏画面重试', 2.2);
}
function safeRequestPointerLock() {
  let res = null;
  try {
    res = renderer.domElement.requestPointerLock?.();
  } catch (e) { onPointerLockFail(e); return; }
  if (res && typeof res.catch === 'function') res.catch(onPointerLockFail);
}
document.querySelectorAll('.m-card').forEach(c =>
